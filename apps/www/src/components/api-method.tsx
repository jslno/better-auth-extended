...
function parseCode(children: JSX.Element) {
	...
	props = props.filter(({ propName, type }) => !!propName && !!type);
...
function Note({ children }: { children: ReactNode }) {
	return (
		<div className="flex relative flex-col gap-2 p-3 w-full break-words rounded-none border text-md text-wrap border-border bg-fd-secondary/50">
